...
open List CoxeterMatrix CoxeterSystem

namespace CoxeterGroup.Bruhat
...
def lt_adj (u w : W) := ∃ t ∈ T, w = u * t ∧ ℓ(u) < ℓ(w)

def lt_adj' (u w : W) := ∃ t ∈ T, w = t * u ∧ ℓ(u) < ℓ(w)

lemma lt_adj_iff_lt_adj' {u w : W} : lt_adj cs u w ↔ lt_adj' cs u w := by sorry
  /- constructor
...
def Lt (u w : W) : Prop := Relation.TransGen (Bruhat.lt_adj cs) u w

def Le (u w : W) : Prop := Relation.ReflTransGen (Bruhat.lt_adj cs) u w

/- instance LT : LT W where
  lt := Bruhat.lt cs

instance LE : LE W where
  le := Bruhat.le cs -/

lemma length_le_of_le {u w : W} (h : Le cs u w) : ℓ(u) ≤ ℓ(w) := by
  rw [Le] at h
  induction h with
  | refl => exact le_rfl
  | tail _ bltc uleb =>
...
lemma length_lt_of_lt {u w : W} (h : Lt cs u w) : ℓ(u) < ℓ(w) := by
  rw [Lt] at h
  induction h with
  | single ultb =>
...
lemma lt_of_le_of_length_lt {u w : W} (h : Le cs u w) (ht : ℓ(u) < ℓ(w)) : Lt cs u w := by
  induction h with
  | refl => by_contra; exact lt_irrefl _ ht
  | tail hyp bltc _ => exact Relation.TransGen.tail'_iff.mpr ⟨_, hyp, bltc⟩

lemma eq_of_le_of_length_ge {u w : W} : Le cs u w → ℓ(u) ≥ ℓ(w) → u = w := by sorry
  /- intro ulew ugew
  rcases ulew with (_ | ⟨uleb, b, ⟨_, _, bltw⟩⟩)
  · rfl
  · by_contra; linarith [length_le_of_le uleb, bltw, ugew] -/

instance instPartialOrder : PartialOrder W where
  le := Le cs
  lt := Lt cs
  ...
  le_antisymm := fun (u w : W) ulew wleu => sorry
    --eq_of_le_of_length_ge ulew (length_le_of_le wleu)


def interval (x y : W) : Set W :=
  haveI : PartialOrder W := instPartialOrder cs
  Set.Icc x y
...
#check right_exchange'

example {L : List B} (n : ℕ) : L.eraseIdx n = L.removeNth n := by
  revert n
  induction L with
  | nil => intros; rfl
  | cons α L h =>
    ...
    match n with
      | 0 => rfl
      | n + 1 =>
        simp only [eraseIdx, removeNth, h (Nat.add n 0)]

--  Bjorner, Brenti, Lemma 2.2.1
lemma exists_intermediate_reduced_subword {L L' : List B} (hne: π L ≠ π L') (hr: cs.IsReduced L) (hr': cs.IsReduced L') (hs: List.Sublist L' L) : ∃ (L'' : List B), cs.IsReduced L'' ∧ Lt cs (π L') (π L'') ∧ ℓ((π L'')) = ℓ((π L')) + 1
    ∧ List.Sublist L'' L := by
...
lemma le_of_exists_reduced_subword {L L' : List B} (hr : cs.IsReduced L) (hr' : cs.IsReduced L') (hs : L'.Sublist L) (n : ℕ) (h : ℓ((π L)) = ℓ((π L')) + n) : Le cs (π L') (π L) := by sorry
  /- induction' n with n hn
  · rw [Nat.zero_eq, add_zero] at h
    rw [hs.eq_of_length (((length_eq_iff.mp hr).trans h).trans (length_eq_iff.mp hr').symm).symm]
  · by_cases hne : (L : G) = L'
    · rw [hne]
    · rcases exists_intermediate_reduced_subword hne hr hr' hs with ⟨L'', h⟩
      sorry -/

-- Subword Property
theorem le_iff_exists_reduced_subword {L: List B} (hr : cs.IsReduced L) : Le cs u (π L) ↔ ∃ (L': List B), cs.IsReduced L' ∧ List.Sublist L' L ∧ u = π L' where
  mp := by
    sorry
  mpr := fun
    | .intro w h => by
...
instance interval.fintype {x y : W} : Fintype (interval cs x y) where
  elems := by
...
noncomputable instance : haveI : PartialOrder W := instPartialOrder cs; LocallyFiniteOrder W :=
  sorry
  --LocallyFiniteOrder.ofFiniteIcc (fun _ _ => Finite.of_fintype _)
/-
instance : OrderBot W where
  bot := 1
  bot_le a := sorry

lemma BruhatOrder.adj (u w : W) (h: u ⋖ w) : u * w⁻¹ ∈ T := sorry
 -/
end Bruhat
