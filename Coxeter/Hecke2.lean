...
-- lemma Hecke.mul_zero1 : ∀ (a : Hecke G), a * 0 = 0 := by{
--   intro a
--   simp [HeckeMul]
-- }
...
lemma Hecke.mul_assoc :∀ (a b c : Hecke G), HeckeMul (HeckeMul a b) c = HeckeMul a (HeckeMul b c):=by
  intro a b c
  simp only [HeckeMul]
  rw [LinearEquiv.toFun_eq_coe,←(@Function.comp_apply _ _ _ (LinearEquiv.invFun (alg_hom G))),(LinearEquiv_invFun_comp_Fun (LaurentPolynomial ℤ) (alg_hom G))]
  simp only[id]
  rw [(Subalgebra.toSemiring (subalg G)).mul_assoc,←(@Function.comp_apply _ _ _ (LinearEquiv.invFun (alg_hom G))),(LinearEquiv_invFun_comp_Fun (LaurentPolynomial ℤ) (alg_hom G))]
  simp only [id]
