...
variable {B : Type*}
variable (s : B → G)
local prefix:100 "π" => @List.wordProd B G _ s

/-- Definition for order-2 generated group:
An order-2 generated group is a group `G` generated by a subset `S` of involutions. -/
class OrderTwoGen where
  order_two : ∀ (i : B), s i * s i = 1 ∧ s i ≠ 1
  expression : ∀ (i : B), ∃ (L : List B), s i = π L

-- instance : OrderTwoGen (Set.range cs.simple) where
--   order_two := by intro w hw; constructor;
--   expression := by sorry

namespace OrderTwoGen
variable {B : Type*}
variable {G : Type*} [Group G] {s : B → G} [h : OrderTwoGen s]


@[simp]
lemma gen_square_eq_one : ∀ i : B, s i * s i = 1 := fun i => (h.1 i).1

@[simp]
lemma gen_square_eq_one' (i : B) : s i * s i = 1:= by simp [gen_square_eq_one i]
