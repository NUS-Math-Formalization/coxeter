...
--open Classical
variable {B : Type*}
variable {G : Type*} [instGroup : Group G]
variable {s : B → G}
...
@[coe]
abbrev List.wordProd (L : List B) : G := prod (map s L)

local prefix:100 "π" => @wordProd B G instGroup s

instance List.ListGtoGroup : CoeOut (List G) G where
  coe := fun L ↦ prod L

--instance List.ListStoGroup  {B:Type*} {G: Type*} [ig:Group G] {s:B→ G}: CoeOut (List B) G where
--  coe := fun L ↦ @wordProd B G ig s L

@[simp, gprod_simps]
lemma wordProd_nil : π [] = 1 := by exact List.prod_nil

@[simp, gprod_simps]
lemma wordProd_singleton {i : B}: π ([i]) = s i := by
  simp [wordProd]

@[simp, gprod_simps]
lemma wordProd_eq_of_list_eq {L1 L2 : List B} (h : L1 = L2) : π L1 = π L2 := by rw [h]

-- instance HasHMulListS : HMul (List B) B G where
--   hMul := fun L g ↦ π L * g
...
@[gprod_simps]
lemma wordProd_cons (hd : B)  (tail : List B) : π (hd :: tail) = π [hd] * π (tail) := by simp [wordProd]

@[simp, gprod_simps]
lemma wordProd_append {l1 l2 : List B} : π (l1 ++ l2) =  π l1 * π l2 := by simp [wordProd]

@[simp, gprod_simps]
lemma wordProd_append_singleton {l1 : List B} {i : B} : π (l1 ++ [i]) = π l1 * π [i] := by simp [wordProd]



@[simp]
abbrev inv_reverse (s : B → G) (L : List B) : List G := (map (fun i ↦ (π [i])⁻¹) L).reverse

-- lemma wordProd_inv_eq_inv_reverse (L : List B) : (π L)⁻¹ = prod (inv_reverse s L) := by rw [List.prod_inv_reverse]
