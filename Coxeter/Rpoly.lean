...
-- trans to ...
lemma length_induction {p : G → Prop} {w: G} {s:rightDescent w} (hw: Nonempty (rightDescent w)) (h1 : p 1) (hws : p (w*s) → p w) : ∀ u:G, p u := sorry
...
lemma le_cancel_of_mem_rD {u v :G} {s: hG.S} (h: s.1 ∈ rightDescent v) : u*s ≤ v*s → u ≤ v := by
  sorry

-- trans to Hecke
lemma muls_apply_antidiagonal_of_memrD (h : Hecke G) (s : hG.S) (w : G) (h1 : ℓ(w*s) < ℓ(w)) :
    (h * TT s.1) w = (q-1) * h w + h (w*s) := sorry
...
lemma Rpoly_aux {u v :G} (h1:s ∈ rightDescent v) (h2:s ∈ rightDescent u):
    (TTInv v⁻¹) u * q = (TTInv (v * s)⁻¹) (u * s) := by
      ...
      --rw [TTInv_s_eq,mul_sub (TTInv (v * s)⁻¹) (q⁻¹ • TT s) (1 - q⁻¹) • 1]
      sorry

lemma Rpoly_memrD : ∀(u v:G),s ∈ rightDescent v → s ∈ rightDescent u → R G u v = R G (u*s) (v*s) := by
  intro u v h1 h2
...
-- lemma Rpoly_not_le_aux : ∀ l, ∀(u v:G), l = ℓ(v) → R G u v ≠ 0 → u ≤ v := by
--   intro l
--   intro u v hl
...
--   · by_cases hn' : Nonempty (rightDescent u)
--     · rw [R,R]
--       have hlvs : ℓ(v*s) + 1 = ℓ(v) := sorry
--       by_cases huv : u ≤ v
--       · have usvs : u * s ≤ v * s := sorry
--         have t1:= eq_true huv
--         have t2:= eq_true usvs
--         simp [t1,t2]
--         rw [←mul_inv_rev,←Rpoly_aux h1 h2]
--         have hlusv : ℓ(v) + ℓ(u) = ℓ(v*s) + ℓ(u*s) + 2:=sorry
--         rw [hlusv,pow_add,neg_one_pow_two,mul_one,←hlvs,pow_add q,pow_one]
--         ring
--       · have usvs : ¬(u * s ≤ v * s) := sorry
--         have f1 := eq_false huv
--         have f2 := eq_false usvs
--         simp [f1,f2]
--     · have : Nonempty (rightDescent u) := Nonempty.intro ⟨s,h2⟩
--       contradiction
--   · have : Nonempty (rightDescent v) := Nonempty.intro ⟨s,h1⟩
