...
@[simp]lemma length_mul_generator_of_length_lt (w:G) (s:S) : ℓ(w*s) < ℓ(w) ↔ ℓ(w*s) = ℓ(w)-1:= lr_symm (motive := fun (n:ℕ) (m:ℕ) => (m < n ↔ m = n - 1 )) _ _ length_generator_mul_of_length_lt

lemma length_mul_generator_of_length_gt (w:G) (s:S) : ℓ(w) < ℓ(w*s) ↔ ℓ(w*s) = ℓ(w)+1:=sorry

lemma reduced_word_of_generator_mul_of_length_gt {s:S} {w:G} {L:List S} (h:reduced_word L) (heq:w=L.gprod) :
ℓ(w) < ℓ(s*w) → reduced_word (s::L) ∧ (s::L).gprod = s*w := by
   intro h1
...
lemma reduced_word_of_mul_generator_of_length_gt {w:G} {s:S} {L:List S} (h:reduced_word L) (heq:w=L.gprod) :
   ℓ(w) < ℓ(w*s) → reduced_word (L++[s]) ∧ (L++[s]).gprod = w*s := by
      sorry
...
lemma length_mul_of_mem_D_R (w:G) (h:w≠1) (h2:s ∈ D_R w) : ℓ(w*s) = ℓ(w) -1 :=
   sorry
   -- lr_symm (motive := fun (n:ℕ) (m:ℕ) => )
