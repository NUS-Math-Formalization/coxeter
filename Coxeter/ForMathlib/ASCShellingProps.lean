...
def Link (F : AbstractSimplicialComplex V) (x : V) : Set F.faces :=
  {f | x ∉ f.1 ∧ {x} ∪ f.1 ∈ F.faces }

def LinkSimplex (F : AbstractSimplicialComplex V) (x : V) (h : {x} ∈ F.faces) : AbstractSimplicialComplex V where
  faces := Link F x
  empty_mem := by
...
/-def VertexDecomposable [Fintype V] (F : AbstractSimplicialComplex V) (hp : Pure F) : Prop :=
  (∃ (f : Finset V), F = simplex f) ∨
  (∃ (x : V), (DeletionSimplex F x).rank = F.rank ∧ VertexDecomposable (DeletionSimplex F x) ∧
  (LinkSimplex F x).rank + 1 = F.rank)-/
