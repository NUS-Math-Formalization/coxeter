 ...
 _ =  List.prod (List.reverse tail) * (hd * hd * tail.prod):= by {
  rw [mul_assoc tail.reverse.prod, <-mul_assoc hd]} 
   _ =  tail.reverse.prod * tail.prod  := by { 
    have oo := order_two hd (cons_hd_subsetList h)
    rw [oo]
    simp 
   }
   _ = 1 := ih  (cons_tail_subsetList h) 
 }
} 
...
:=  
mul_eq_one_iff_eq_inv.1 (reverse_prod_prod_eq_one S order_two h)
...
ℓ(g⁻¹)  =   ℓ(g)
:= by {
    apply le_antisymm (length_le_inv_length S _ _ g) 
    have gg : g⁻¹⁻¹ = g:= by {simp}
    have := length_le_inv_length S order_two gen (g⁻¹) 
    rw [gg] at this 
    exact this
}
